...
pr_err("tas5731_set_dai_sysclk\n");
	...
	if (freq == 512 * 48000)
		regmap_write(priv->regmap, TAS5731_LOCK_CTL, 0x74);
	else
		regmap_write(priv->regmap, TAS5731_LOCK_CTL, 0x6c);*/
	return 0;
}
...
pr_err("tas571x_i2c_probe\n");
	...
	ret = regmap_write(priv->regmap, TAS571X_OSC_TRIM_REG, 0x00);
	msleep(500);
	regmap_read(priv->regmap, 0x1b, &j);
	pr_err("0x1b read: %d\n",j);
	
	
	ret = regmap_write(priv->regmap, 0x20, 0x00887772);
	ret = regmap_write(priv->regmap, 0x19, 0x3a);
	ret = regmap_write(priv->regmap, 0x25, 0x01012345);
	ret = regmap_write(priv->regmap, 0x07, 0x30);
	ret = regmap_write(priv->regmap, 0x08, 0x30);
	ret = regmap_write(priv->regmap, 0x09, 0x30);
	ret = regmap_write(priv->regmap, 0x04, 0x06);
	ret = regmap_write(priv->regmap, 0x00, 0x74);
	ret = regmap_write(priv->regmap, 0x05, 0x02);
